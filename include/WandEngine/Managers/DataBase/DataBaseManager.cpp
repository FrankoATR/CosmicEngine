...
bool DataBaseManager::OpenDatabaseForSaving(const std::string &newDbName)
{
    if(db)
    {
        sqlite3_close(db);
...
bool DataBaseManager::OpenDatabaseForLoading(const std::string &newDbName)
{
    if(db)
    {
        sqlite3_close(db);
    ...
    std::cout << "Base de datos abierta para cargar: " << dbName << std::endl;
    ExecuteSQL("PRAGMA busy_timeout = 5000;");
    return true;
}
