...
namespace WandEngine
{
    ...
    CollisionSide GameGridCollisions::GetCollisionSide(GameBodyObject *body1, GameBodyObject *body2)
    {
        ...
        if (minDelta == deltaLeft) return CollisionSide::LEFT;
        if (minDelta == deltaRight) return CollisionSide::RIGHT;
        if (minDelta == deltaTop) return CollisionSide::TOP;
        if (minDelta == deltaBottom) return CollisionSide::BOTTOM;
